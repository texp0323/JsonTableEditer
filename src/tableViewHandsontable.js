...
export function displayDataWithHandsontable(data, dataKeyName, config) {
    ...
    hotInstance = new Handsontable(container, {
            ...
            items: {
                ...
                "duplicate_row": {
                    ...
                    callback: async function(key, selection, event) {
                        ...
                        if (Array.isArray(data)) {
                            ...
                            const originalKey = Object.keys(data)[r];
                            if (originalKey === undefined) return;
                ...
                "duplicate_col": {
                    ...
                    hidden: function() {
                        const selection = this.getSelectedRangeLast();
                        if (!selection) return true;
                        if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'object' && data[0] !== null) return false;
        ...
        afterChange: function(changes, source) {
            ...
            changes.forEach(([row, prop, oldValue, newValue]) => {
                ...
                const hotDataStructureInfo = {
                    ...
                    headers: currentInstanceHeaders,
        ...
        afterCreateRow: function(index, amount, source) {
            ...
            if (Array.isArray(parentData)) {
                for (let i = 0; i < amount; i++) {
                    let newItem = null;
                    if (Array.isArray(currentInstanceHeaders) && currentInstanceHeaders.length > 0 && currentInstanceHeaders[0] !== "Index") {
                        newItem = {};
                        currentInstanceHeaders.forEach(header => {
                            if (typeof header === 'string') newItem[header] = null;
                        });
                    } else if (parentData.length > 0 && typeof parentData[0] === 'object' && parentData[0] !== null && !Array.isArray(parentData[0])) {
                        newItem = {};
        ...
        afterCreateCol: function(index, amount, source) {
            ...
            if (Array.isArray(currentTableData) && currentTableData.length > 0 && typeof currentTableData[0] === 'object' && currentTableData[0] !== null) {
                for (let i = 0; i < amount; i++) {
                    Swal.fire({
                ...
                Swal.fire('알림', '이 뷰에서는 "열 추가"가 "새 키-값 쌍 추가"와 동일하게 동작합니다.', 'info');
                Swal.fire({
                    title: '새 항목 키 입력',
                    input: 'text',
                    inputPlaceholder: '새로운 키 이름을 입력하세요',
        ...
        beforeRemoveCol: function(index, amount, physicalCols, source) {
            ...
            if (Array.isArray(currentTableData) && currentTableData.length > 0 && typeof currentTableData[0] === 'object' && currentTableData[0] !== null) {
                ...
                Swal.fire('알림', '이 뷰에서 특정 열만 삭제하는 것은 지원되지 않습니다. 행 전체(키-값 쌍)를 삭제해주세요.', 'info');
                return false;
            } else {
